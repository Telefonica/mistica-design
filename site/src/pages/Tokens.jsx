...
const Tokens = () => {
  fetch(
    "https://raw.githubusercontent.com/Telefonica/mistica-design/production/tokens/telefonica.json"
  )
    .then((res) => res.json())
    .then((res) => {
      ...
      const constantDescription = Object.values(mode[0]).map(
        (v) => v.description
      );

      const colorName = constantValues[1].substring(
        constantValues[1].indexOf(".") + 1,
        constantValues[1].length - 1
      );
      ...
      constantValues.forEach((item) => {
        if (!item.includes("rgba")) {
          const colorName = item.substring(
            item.indexOf(".") + 1,
            item.length - 1
          );
          const colorValue = res.global.palette[colorName].value;
  ...
  return (
    <ResponsiveLayout>
      ...
      <Box paddingY={48}>
        <div data-theme={theme.isDarkMode ? "dark" : "light"}>
          <div id={`table`}>
            <div id={`col1`}>
              <div id="consValues"></div>
            </div>
            <div id="col2">
              <div id="consNames"></div>
            </div>
            <div id="col3">
              <div id="paletteValues"></div>
            </div>
            <div id="col4">
              <div id="consDescription"></div>
            </div>
          </div>
        </div>
      </Box>
